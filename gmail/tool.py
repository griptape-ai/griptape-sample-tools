    ...
    @activity(
        config={
            ...
            "schema": Schema(
                {
                    Literal(
                        "max_results", description="Max number of emails to return. Defaults to 10."
                    ): int
                }
            ),
        }
    )
    def list_unread_emails(self, max_results: int = 10) -> ListArtifact:
        ...
        delegated_credentials = credentials.with_subject(
            os.getenv("GOOGLE_DELEGATED_EMAIL")
        )
